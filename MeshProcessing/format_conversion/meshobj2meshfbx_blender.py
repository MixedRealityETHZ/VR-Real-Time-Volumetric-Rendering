...
def convert_format_and_get_stats(file, input_directory, output_directory):
    ...
    output_path = os.path.join(output_directory, file.lower().replace('.obj', '.fbx'))
...
def batch_convert_format(input_directory, output_directory):
    ...
    # Get selected files
    selected_files = [f for f in os.listdir(input_directory) if f.endswith('.obj') and is_selected(f)]
